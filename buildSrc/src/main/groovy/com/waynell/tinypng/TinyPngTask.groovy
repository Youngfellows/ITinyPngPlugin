...
public class TinyPngTask extends DefaultTask {
        ...
        for (File file : resDir.listFiles()) {
            ...
            if (fileName.endsWith(".jpg") || fileName.endsWith(".png")) {
                ...
                try {
                    ...
                    println "压缩前图片大小: ${beforeSizeStr} ,图片: ${resDir}\\${fileName}"

                    // Use the Tinify API client
                    def tSource = Tinify.fromFile("${resDir}/${fileName}")
                    println "使用tinify api 进行图片压缩..."
    ...
    @TaskAction
    def run() {
        println("run==>>> " + configuration.toString())
        ...
        configuration.resourceDir.each { d ->
            def dir = new File(d)
            println "resourceDir: ${dir.getAbsolutePath()}"
            println "dir.exists(): ${dir.exists()} ,dir.isDirectory(): ${dir.isDirectory()}"

            if (dir.exists() && dir.isDirectory()) {
                if (!(configuration.resourcePattern ?: false)) {
                    configuration.resourcePattern = ["drawable[a-z-]*"]
                }
                println "resourcePattern: ${configuration.resourcePattern.toString()}"
